...
class Node:
    def __init__(self, executor, args, requests):
        """
        Node in river
    ...
    def fetch_config(self):
        ...
        url = f'http://95.179.226.113:5000/get-config?cluster_id=' \
              f'{self.cluster_id}'
        cfg = self.requests.get(url=url).json()
        self.set_config(cfg)
    ...
    def heartbeat(self):
        ...
        self.requests.post(url=host_url)
    ...
    def get_data(self, server):
        ...
        headers = {'Content_type': 'application/json',
                   ...
                   'term': str(self.term),
                   'status': self.status}
        self.requests.get(url=follower_url, headers=headers, timeout=2)
    ...
    def send_config(self, server):
        ...
        headers = {'Content_type': 'application/json',
                   ...
                   'term': str(self.term),
                   'status': self.status}
        self.requests.post(url=follower_url, json=self.config, headers=headers)
    ...
    def request_vote(self, url):
        ...
        """
        response = int(self.requests.get(url + '/requestvote',
    ...
    def get_auth_token(self):
        ...
        r = self.requests.post(url=url, data=token_data, headers=token_headers)
